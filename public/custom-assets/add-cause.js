...
function docUpload() {
    ...
    $('.upload__inputfile2').each(function () {
        $(this).on('change', function (e) {
            imgWrap = $(this).closest('.upload__box2').find('.upload__img-wrap2');
            var maxLength = $(this).attr('data-max_length');
            ...
            filesArr.forEach(function (f, index) {
                ...
                if (imgArray.length > maxLength) {
                    ...
                    if (len > maxLength) {
                        ...
                        var reader = new FileReader();
                        reader.onload = function (e) {
                            var html = "<div class='upload__img-box2'><div style='background-image: url(" + e.target.result + ")' data-number='" + $(".upload__img-close").length + "' data-file='" + f.name + "' class='img-bg'><div class='upload__img-close'></div></div></div>";
                            imgWrap.append(html);
                            iterator++;
                        }
                        reader.readAsDataURL(f);
